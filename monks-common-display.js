...
export class MonksCommonDisplay {
    ...
    patchFunc(
      ...
      async function (wrapped, ...args) {
        ...
        if (!!MonksCommonDisplay.selectToken) {
          ...
          let documentId;
          if (game.release.generation < 13) {
            documentId = event.currentTarget.closest('.document').dataset.documentId;
          } else {
            documentId = event.currentTarget.querySelector('.directory-item.entry.document.actor').dataset.dataentryid;
          }
          ...
          if (MonksCommonDisplay.selectToken == 'screen') MonksCommonDisplay.screenChanged();
          else MonksCommonDisplay.focusChanged();
          MonksCommonDisplay.selectToken = null;
          if (MonksCommonDisplay.toolbar && setting('show-toolbar') && game.user.isGM) MonksCommonDisplay.toolbar.render(true);
        } else wrapped(...args);
      },
      'MIXED'
    );
    ...
    patchFunc('Scene.prototype.view', async function (wrapped, ...args) {
      ...
      if (MonksCommonDisplay.playerdata.display || false) {
        if (setting('screen-toggle')) {
          if (MonksCommonDisplay.screenValue == 'gm') MonksCommonDisplay.emit('requestScreenPosition');
          else if (MonksCommonDisplay.screenValue == 'scene') MonksCommonDisplay.sceneView();
          else MonksCommonDisplay.changeScreen();
        }
        if (setting('focus-toggle')) {
          if (MonksCommonDisplay.screenValue == 'gm') MonksCommonDisplay.emit('requestGMTokens');
          else MonksCommonDisplay.changeFocus();
        }
      } else if (game.user.isGM && setting('screen-toggle') && MonksCommonDisplay.screenValue == 'gm' && canvas.scene.active) MonksCommonDisplay.sendScreenMessage('canvasPan', game.canvas.scene._viewPosition);
  ...
  static ready() {
    ...
    if (setting('show-toolbar') && game.user.isGM) MonksCommonDisplay.toolbar = new CommonToolbar().render(true);
  ...
  static requestScreenPosition() {
    if (game.user.isGM && canvas.scene.active && setting('screen-toggle') && MonksCommonDisplay.screenValue == 'gm') MonksCommonDisplay.sendScreenMessage('canvasPan', game.canvas.scene._viewPosition);
  }

  static requestGMTokens() {
    if (game.user.isGM && canvas.scene.active && setting('focus-toggle') && MonksCommonDisplay.focusValue == 'gm') MonksCommonDisplay.sendFocusMessage('controlToken', { tokens: canvas.tokens.controlled.map((t) => t.id), control: true });
  }
  ...
  static dataChange() {
    ...
    if (olddata.display != MonksCommonDisplay.playerdata.display) MonksCommonDisplay.toggleCommonDisplay();
  ...
  static toggleCommonDisplay() {
    if (game.release.generation < 13) {
      ...
      if ((MonksCommonDisplay.playerdata.display || false) && setting('hide-ui')) {
      ...
      for (let id of data.tokens || []) {
        ...
        if (token) {
          if (data.control) {
            ...
            if (token.testUserPermission(game.user, 'OBSERVER') && !token.hidden) token._object?.control({ releaseOthers: false });
          } else {
  ...
  static setScrollTop() {
    if (game.release.generation < 13) {
      ...
      if (active) $('#combat-popout #combat-tracker').scrollTop(active.offsetTop - (setting('limit-shown') > 2 ? 50 : 0));
    } else {
      ...
      const active = tracker.querySelector('.combatant.active');
      if (!active) return;
      let scrollOffset = 0;
  ...
  static changeFocus() {
    if (MonksCommonDisplay.playerdata.display) {
      if (setting('focus-toggle')) {
        ...
        else if (focus == 'combat') tokens = [game.combats.active?.combatant?.token._object];
        else tokens = canvas.scene.tokens.filter((t) => t.id == focus || t.actor?.id == focus).map((t) => t?._object);
  ...
  static getTokens(value) {
    ...
    if (value == 'party') return canvas.scene.tokens.filter((t) => t.testUserPermission(game.user, 'LIMITED') && !t.hidden && !MonksCommonDisplay.isDefeated(t));
  ...
  if (game.user.isGM) {
    if (game.release.generation < 13) {
      ...
      btn.on('click', () => {
        ...
        game.settings.set('monks-common-display', 'show-toolbar', toggled);
        if (toggled) {
          if (!MonksCommonDisplay.toolbar) MonksCommonDisplay.toolbar = new CommonToolbar().render(true);
          else MonksCommonDisplay.toolbar.render(true);
        } else {
          if (MonksCommonDisplay.toolbar) MonksCommonDisplay.toolbar.close({ properClose: true });
        }
      ...
      const buttonExist = document.querySelector('button.fas.fa-chalkboard-teacher');
      if (buttonExist === null) {
        const name = 'monkscommondisplay';
        const title = 'Toggle Common Display Bar';
        const icon = 'fas fa-chalkboard-teacher';
...
Hooks.on('controlToken', async (token, control) => {
  ...
  if (display && setting('focus-toggle')) {
    ...
    let shouldControl = (focus == 'gm' && MonksCommonDisplay.gmControlledTokens.has(token.id)) || (focus == 'combat' && game.combats.active && game.combats.active.combatant?.token.id == token.id) || focus == token.id || focus == token.actor?.id;
...
Hooks.on('updateToken', async function (document, data, options, userid) {
  ...
  if (display && (data.x != undefined || data.y != undefined) && setting('screen-toggle') && !document.hidden) {
...
Hooks.on('updateCombat', async function (combat, delta) {
  let display = MonksCommonDisplay.playerdata.display || false;
  if (display && combat.started && combat.active && combat.combatant?.token && !combat.combatant.token.hidden) {
    if (setting('screen-toggle') && MonksCommonDisplay.screenValue == 'combat') {
      MonksCommonDisplay.changeScreen();
    }

    if (setting('focus-toggle') && MonksCommonDisplay.focusValue == 'combat' && combat.combatant?.token.isOwner) {
...
Hooks.on('targetToken', async function (user, token, targeted) {
  if (game.combats.viewed?.started && game.combats.viewed?.active && MonksCommonDisplay.screenValue == 'combat') {
    window.setTimeout(() => {
      MonksCommonDisplay.changeScreen();
    }, 100);
  }
...
Hooks.on('renderCombatTracker', async (dialog) => {
  if (MonksCommonDisplay.playerdata.display && setting('show-combat')) {
    if (game.release.generation > 12) {
      ...
      if (combatApp !== undefined && dialog.isPopout && activeCombat !== undefined) {
        if (setting('show-combatants')) {
          const singleCombatant = combatApp.querySelector('li.combatant').clientHeight;
        ...
        if (!setting('show-combatants') || setting('limit-shown') === 1) {
          const nonActiveCombatants = combatApp.querySelectorAll('.combatant:not(.active)');
