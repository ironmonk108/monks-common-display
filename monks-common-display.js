...
export let error = (...args) =>
  console.error("monks-common-display | ", ...args);
export let i18n = (key) => {
...
export let patchFunc = (prop, func, type = "WRAPPER") => {
  const nonLibWrapper = () => {
    ...
    const context = parts.reduce((obj, key) => obj?.[key], globalThis);

    if (!context || typeof context[finalProp] !== "function") {
      console.warn(`[patchFunc] Cannot find function to patch at: ${prop}`);
      return;
    }

    const oldFunc = context[finalProp];

    context[finalProp] = function (...args) {
      if (type !== "OVERRIDE") {
        return func.call(this, oldFunc.bind(this), ...args);
      } else {
  ...
  if (game.modules.get("lib-wrapper")?.active) {
    try {
      ...
      console.warn(`[patchFunc] libWrapper failed, falling back.`, e);
      nonLibWrapper();
    }
  } else {
    nonLibWrapper();
...
export class MonksCommonDisplay {
  ...
  static init() {
    ...
    if (game.modules.get("lib-wrapper")?.active) {
      libWrapper.ignore_conflicts(
        ...
        "monks-active-tiles",
        "ActorDirectory.prototype._onClickEntryName",
      );
    }
    ...
    patchFunc(
      "Journal.prototype.constructor.showImage",
      async function (wrapped, ...args) {
        let [src, data] = args;
    ...
    patchFunc(
      "ImagePopout.prototype.constructor._handleShareImage",
      async function (wrapped, ...args) {
        let [options] = args;
    ...
    patchFunc(
      ...
      async function (wrapped, ...args) {
        ...
        if (!!MonksCommonDisplay.selectToken) {
          ...
          if (game.release.generation < 13) {
            documentId =
              event.currentTarget.closest(".document").dataset.documentId;
          } else {
            documentId = event.currentTarget.querySelector(
              ".directory-item.entry.document.actor",
            ).dataset.dataentryid;
          }
          ...
          if (
            MonksCommonDisplay.toolbar &&
            setting("show-toolbar") &&
            game.user.isGM
          )
    ...
    patchFunc(
      "PlayerList.prototype._getUserContextOptions",
    ...
    patchFunc("Scene.prototype.view", async function (wrapped, ...args) {
      ...
      if (MonksCommonDisplay.playerdata.display || false) {
        if (setting("screen-toggle")) {
          ...
          else MonksCommonDisplay.changeScreen();
        }
        if (setting("focus-toggle")) {
          if (MonksCommonDisplay.screenValue == "gm")
            MonksCommonDisplay.emit("requestGMTokens");
          else MonksCommonDisplay.changeFocus();
        }
      } else if (
        game.user.isGM &&
        setting("screen-toggle") &&
        MonksCommonDisplay.screenValue == "gm" &&
        canvas.scene.active
      )
        MonksCommonDisplay.sendScreenMessage(
          "canvasPan",
          game.canvas.scene._viewPosition,
        );
  ...
  static ready() {
    ...
    if (
      !game.modules.get("monks-combat-details")?.active &&
      !game.modules.get("monks-enhanced-journal")?.active
    ) {
      patchFunc(
        "Draggable.prototype._onDragMouseUp",
        async function (wrapped, ...args) {
          try {
            if (this.app.constructor._getInheritanceChain) {
              ...
              Hooks.callAll(
                `dragEnd${this.app.constructor.name}`,
                this.app,
                this.app.position,
              );
            }
  ...
  static requestGMTokens() {
    if (
      ...
      MonksCommonDisplay.focusValue == "gm"
    )
      MonksCommonDisplay.sendFocusMessage("controlToken", {
        tokens: canvas.tokens.controlled.map((t) => t.id),
        control: true,
      });
  }
  ...
  static dataChange() {
    ...
    MonksCommonDisplay.playerdata = data[game.user.id] || {
      display: false,
      mirror: false,
      selection: false,
    };
    ...
    game.settings.set(
      "monks-common-display",
      "startupdata",
      MonksCommonDisplay.playerdata.display,
    );
  ...
  static toggleCommonDisplay() {
    if (game.release.generation < 13) {
      let display =
        (MonksCommonDisplay.playerdata.display || false) && setting("hide-ui");
      $("body")
        .toggleClass("hide-ui", display)
        .toggleClass("hide-chat", display && !setting("show-chat-log"))
        .toggleClass(
          "hide-camera-views",
          display && !setting("show-camera-views"),
        )
      ...
      if (display && ui.sidebar) ui.sidebar.activateTab("chat");
      //$("body").get(0).style.setProperty("--combat-popout-scale", display ? setting('combat-scale') : 1);
    } else {
      if (
        (MonksCommonDisplay.playerdata.display || false) &&
        setting("hide-ui")
      ) {
        const nav = ui.nav.element;
        nav.style.display = "none";
        const controls = ui.controls.element;
        controls.style.display = "none";
        const hotbar = ui.hotbar.element;
        hotbar.style.display = "none";
        const activWindows = ui.activeWindow?.element;
        if (activWindows !== undefined && activWindows.id !== "combat-popout") {
          console.log(activWindows);
          activWindows.style.display = "none";
        }
        const players = ui.players.element;
        players.style.display = "none";
        const sideBar = ui.sidebar.element;
        if (setting("show-chat-log")) {
          const sidebraButtons = sideBar.querySelector(".tabs.faded-ui");
          sidebraButtons.style.display = "none";
          const sideVisible = sideBar.querySelector("#sidebar-content");
          sideVisible.className = "flexcol active-chat expanded";
          const userPartOfChat = sideBar.querySelector(".chat-form");
          userPartOfChat.style.display = "none";
          const chat = ui.chat;
          chat.render(true);
        } else {
        ...
        setTimeout(() => {
          const combatTracker = game.combats.apps[0].popout;
          if (combatTracker !== undefined) {
            if (setting("show-combat") || setting("show-combatants")) {
              combatTracker.render(true);
              combatTracker.setPosition({ top: 0, left: 0 });
            } else {
              combatTracker.close();
            }
          }
  ...
  static canvasPan(data) {
    if (MonksCommonDisplay.playerdata.display) {
      if (data.animate) canvas.animatePan(data);
      else canvas.pan(data);
    }
  }

  static controlToken(data) {
    if (MonksCommonDisplay.playerdata.display) {
      if (data.overwrite) MonksCommonDisplay.gmControlledTokens.clear();
      ...
      for (let id of data.tokens || []) {
        ...
        if (token) {
          if (data.control) {
            ...
            if (
              token.testUserPermission(game.user, "OBSERVER") &&
              !token.hidden
            )
  ...
  static setScrollTop() {
    if (game.release.generation < 13) {
      ...
      if (active)
        $("#combat-popout #combat-tracker").scrollTop(
          active.offsetTop - (setting("limit-shown") > 2 ? 50 : 0),
        );
    } else {
    }
  }
    ...
    if (MonksCommonDisplay.playerdata.display && setting("screen-toggle")) {
      ...
      if (tokens && tokens.length) {
        ...
        for (let token of tokens) {
          ...
          x2 = !x2
            ? x + token.width * canvas.dimensions.size
            : Math.max(x2, x + token.width * canvas.dimensions.size);
          y2 = !y2
            ? y + token.height * canvas.dimensions.size
            : Math.max(y2, y + token.height * canvas.dimensions.size);
        }

        if (setting("show-chat-log")) x2 += $("#sidebar").width() / 2;
        ...
        let screenWidth =
          $("body").width() -
          (setting("show-chat-log") ? $("#sidebar").width() : 0);
        let ratio = screenWidth / $("body").height();
        let width = Math.max(
          x2 - x1 + canvas.dimensions.size,
          setting("focus-padding") * ratio * canvas.dimensions.size,
        );
        let height = Math.max(
          y2 - y1 + canvas.dimensions.size,
          setting("focus-padding") * canvas.dimensions.size,
        );
        ...
        if (
          panData.x != canvas.scene._viewPosition.x ||
          panData.y != canvas.scene._viewPosition.y
        ) {
          panData.speed = 250;
        } else {
  ...
  static sceneView() {
    if (MonksCommonDisplay.playerdata.display && setting("screen-toggle")) {
      ...
      let scaleHeight =
        $("body").height() / canvas.scene.dimensions.sceneHeight;
      let panData = {
        x:
      ...
      if (setting("focus-toggle")) {
        ...
        if (focus == "gm")
          tokens = MonksCommonDisplay.gmControlledTokens.filter((t) => {
            ...
            return (
              token.testUserPermission(game.user, "OBSERVER") && !token.hidden
            );
          });
        else if (focus == "combat")
          tokens = [game.combats.active?.combatant?.token._object];
        else
          tokens = canvas.scene.tokens
            .filter((t) => t.id == focus || t.actor?.id == focus)
            .map((t) => t?._object);

        canvas.tokens.releaseAll();
        if (tokens.length) {
          if (game.release.generation < 13) {
            for (let token of tokens) token.control({ releaseOthers: false });
          } else {
            for (let token of tokens) token.control({ groupSelection: true });
          }
        }
      } else {
        canvas.tokens.releaseAll();
  ...
  static getTokens(value) {
    if (
      ...
      !game.combats.active.combatant.hidden &&
      !game.combats.active.combatant?.token.hidden
    ) {
    ...
    if (value == "party")
      return canvas.scene.tokens.filter(
        (t) =>
          t.testUserPermission(game.user, "LIMITED") &&
          !t.hidden &&
          !MonksCommonDisplay.isDefeated(t),
      );

    let ids = value.split(",").filter((i) => /^[a-zA-Z0-9]{16}$/.test(i));
    return canvas.scene.tokens.filter(
      (t) => (ids.includes(t.id) || t.actor?.id == value) && !t.hidden,
    );
  }
}
  ...
  if (MonksCommonDisplay.playerdata.display && setting("show-combat")) {
    if (delta.round === 1 && combat.turn === 0 && combat.started === true) {
      ...
      if (game.release.generation < 13) {
        ...
        combatTracker.renderPopout(true);
        setTimeout(() => {
          const combatApp = game.combats.apps[0].popout;
          combatApp.setPosition({ top: 0, left: 0 });
        });
      }
    ...
    if (game.release.generation < 13) {
      ...
      if (tabApp._popout != undefined) {
        ...
        MonksCommonDisplay.closeTimer = setInterval(function () {
          ...
          const tabApp = ui["combat"];
          if (
            MonksCommonDisplay.closeCount > 100 ||
            tabApp._popout == undefined
          ) {
          ...
          if (
            ![states.CLOSING, states.RENDERING].includes(tabApp?._popout._state)
          ) {
      ...
      game.combats.apps[0].popout.close();
    }
  }
  if (MonksCommonDisplay.toolbar && setting("show-toolbar") && game.user.isGM) {
    MonksCommonDisplay.toolbar.render();
...
Hooks.on("renderSceneControls", (control, html, data) => {
  if (game.user.isGM) {
    if (game.release.generation < 13) {
      ...
      const btn = $(
        `<li class="common-display toggle ${game.modules.get("minimal-ui")?.active ? "minimal " : ""}${active ? "active" : ""}" title="${title}" data-tool="${name}"><i class="${icon}"></i></li>`,
      );
      btn.on("click", () => {
        ...
        if (toggled) {
          if (!MonksCommonDisplay.toolbar)
            MonksCommonDisplay.toolbar = new CommonToolbar().render(true);
          else MonksCommonDisplay.toolbar.render(true);
        } else {
          if (MonksCommonDisplay.toolbar)
            MonksCommonDisplay.toolbar.close({ properClose: true });
        }
        $(".main-controls .common-display", html).toggleClass(
          "active",
          toggled,
        );
      });
      ...
      const buttonExist = document.querySelector(
        "button.fas.fa-chalkboard-teacher",
      );
      if (buttonExist === null) {
        const name = "monkscommondisplay";
        const title = "Toggle Common Display Bar";
        ...
        button.type = "button";
        button.className = `control ui-control layer icon ${icon}`;
        ...
        button.addEventListener("click", () => {
          ...
          button.setAttribute("aria-pressed", toggled ? "true" : "false");
        });
        li.appendChild(button);
        const leftControls = html.querySelector("#scene-controls-layers");
        const listItems = leftControls.querySelectorAll("li");
...
Hooks.on("controlToken", async (token, control) => {
  ...
  if (display && setting("focus-toggle")) {
    ...
    let shouldControl =
      (focus == "gm" && MonksCommonDisplay.gmControlledTokens.has(token.id)) ||
      (focus == "combat" &&
        game.combats.active &&
        game.combats.active.combatant?.token.id == token.id) ||
      focus == token.id ||
      focus == token.actor?.id;
...
Hooks.on("renderCombatTracker", async (dialog) => {
  if (MonksCommonDisplay.playerdata.display && setting("show-combat")) {
    if (game.release.generation > 12) {
      const combatApp = game.combats.apps[0].popout?.element;
      const activeCombat = game.combats.apps[0].combats[0]?.active;
      if (
        combatApp !== undefined &&
        dialog.isPopout &&
        activeCombat !== undefined
      ) {
        if (setting("show-combatants")) {
          const singleCombatant =
            combatApp.querySelector("li.combatant").clientHeight;
          const inner = combatApp.querySelector(".combat-tracker.plain");
          const combatsntsToShow = setting("limit-shown");
          inner.style.maxHeight = singleCombatant * combatsntsToShow + "px";
          inner.style.overflow = "hidden";
          const combatTracker = game.combats.apps[0].popout.element;
          const tracker = combatTracker.querySelector(".combat-tracker.plain");
          const allItems = Array.from(tracker.querySelectorAll(".combatant"));
          ...
          if (activeIndex !== -1) {
            const fromActive = allItems.slice(activeIndex);
            const beforeActive = allItems.slice(0, activeIndex);

            const nextRoundLi = document.createElement("li");
            nextRoundLi.style.height = singleCombatant + "px";
            nextRoundLi.classList.add("combatant", "next-round");
            nextRoundLi.innerHTML = `<div class="token-name"style="justify-content: center; display: grid;grid-template-columns: 1fr 1fr 1fr;;"><strong class="name" style="margin: auto;",>${game.i18n.localize("MonksCommonDisplay.nextRound")}</strong></div>`;

            const newOrder = [...fromActive, nextRoundLi, ...beforeActive];
            while (tracker.firstChild) {
              tracker.removeChild(tracker.firstChild);
            }

            newOrder.forEach((li) => tracker.appendChild(li));
          }
        } else {
          const singleCombatant =
            combatApp.querySelector("li.combatant").clientHeight;
          const inner = combatApp.querySelector(".combat-tracker.plain");
          inner.style.maxHeight = singleCombatant + "px";
        }

        if (!setting("show-combatants") || setting("limit-shown") === 1) {
          const nonActiveCombatants = combatApp.querySelectorAll(
            ".combatant:not(.active)",
          );
          nonActiveCombatants.forEach((combatant) => {
            combatant.style.display = "none";
          });
          const activeCombatant = combatApp.querySelector(".combatant.active");
          if (activeCombatant !== null) {
            activeCombatant.style.display = "";
          }
        }
        const combatNav = combatApp.querySelector(".combat-controls");
        combatNav.style.display = "none";
      }
    }
  }
});
